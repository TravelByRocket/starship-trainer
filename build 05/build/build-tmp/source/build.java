...
public void defenseTraining(){ // gameState 21
	...
	if (scene == 0){
		...
		rectMode(CENTER);
		fill(gameRed,50);
		rect(width/2,height/2,height*.8f,width*.2f);
	} else if (scene == 2){
	...
	if (startupDefense && scene == 0){ // create on first loop 
		...
		blastZone = new BlastZone(width*2.5f,height/2,width*1.8f,width*1.9f);
		startupDefense = false; //don't run these initializations again
	...
	if (scene == 0 && blasts.size() > 0){
		...
		for (Asteroid as : asteroids){
			if (blasts.size() > 0 && as.posX > width*0.45f || as.posX < width*0.55f) {
				scene++;
				startupDefense = true;
			} else if (as.posX > width*0.7f){
				as.posX = width/4;
			}
		}
	}
...
class Asteroid{
	...
	public void draw(){
		imageMode(CENTER);
		if (type == 0){
			image(asteroid01, posX, posY, width*0.10f, width*0.10f);
		} else if (type == 1) {
			image(asteroid02, posX, posY, width*0.10f, width*0.10f);
		} else if (type == 2) {
			image(asteroid03, posX, posY, width*0.10f, width*0.10f);
		}
